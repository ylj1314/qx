...
echo "50 19 * * * node /scripts/jd_fanslove.js |ts >> /scripts/logs/jd_fanslove.log 2>&1" >> /scripts/docker/merged_list_file.sh		 # echo "10 9,18 * * * node /scripts/jx_nc.js |ts >> /scripts/logs/jx_nc.log 2>&1" >> /scripts/docker/merged_list_file.sh
